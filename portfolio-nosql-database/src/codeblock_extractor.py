...
class CodeblockExtractor:
        ...
        for name, code in matches:
            ...
            if file_path.exists():
                if overwrite:
                    ...
                    self.logger.debug(f"Arquivo substituído: {file_path}")
                    replaced += 1
                else:
                    self.logger.debug(f"Arquivo ignorado (já existe): {file_path}")
                    ignored += 1
            else:
                file_path.write_text(file_content, encoding="utf-8")
                self.logger.debug(f"Arquivo criado: {file_path}")
                created += 1
    ...
    def extract(self, *, confirm: bool = True) -> None:
        ...
        if not self.md_path.exists():
            self.logger.error(f"Arquivo {self.md_path} não encontrado.")
            return
        if not self.output_dir.exists():
            self.output_dir.mkdir(parents=True)
            self.logger.info(f"Diretório {self.output_dir} criado.")
        self.logger.info("Iniciando extração de codeblocks do arquivo Markdown.")
        matches = self.read_codeblocks()
        total_files = len(matches)
        if not matches:
            self.logger.warning("Nenhum codeblock encontrado.")
            self.logger.warning(f"Arquivo pesquisado: {self.md_path}.")
            return
        self.logger.info(f"Foram encontrados {total_files} codeblocks para extração.")
        if confirm:
            resposta = input(
            ...
            if resposta.strip() != "1":
                self.logger.info("Execução abortada pelo usuário.")
                return
        existing = self.check_existing_files(matches)
        overwrite = True
        if existing and confirm:
            self.logger.info(f"{len(existing)} arquivos já existem na pasta de saída.")
            resposta = input(
                f"➔ {len(existing)} arquivos já existem. Deseja substituir? (1 = sim, 0 = não): "
            )
            overwrite = resposta.strip() == "1"
            if not overwrite:
                self.logger.info("Arquivos existentes serão ignorados.")
        created, replaced, ignored = self.save_codeblocks(matches, overwrite=overwrite)
        self.logger.info(f"Total de arquivos criados: {created}")
        self.logger.info(f"Total de arquivos substituídos: {replaced}")
        self.logger.info(f"Total de arquivos ignorados: {ignored}")
